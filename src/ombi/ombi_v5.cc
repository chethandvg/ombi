/*
 * ombi_v5.cc — OMBI v5: Correctness-Fixed Two-Level Bitmap Buckets
 *
 * Based on ombi_v3.cc with two correctness-fix strategies:
 *
 *   v5 (default):  Adaptive bucket width — bw = minW when minW < BW_MULT.
 *   v5s (-DOMBI_V5S): Sorted-insert — L0 inserts in ascending distance order.
 *
 * All other logic (pool allocator, two-level bitmap, L1 redistribute,
 * cold PQ backup) is identical to v3.
 */
...
/* ----------------------------------------------------------------
 * Pool allocator (identical to v3)
 * ---------------------------------------------------------------- */
...
/* ----------------------------------------------------------------
 * Constructor / Destructor (identical to v3)
 * ---------------------------------------------------------------- */
...
void OmbiQueue::addToL0(int bi, int v, long long d)
{
...
#ifdef OMBI_V5S
    /*
     * SORTED INSERT: Insert in ascending distance order within the bucket's
     * linked list. This ensures extractFirstLiveL0 always returns the vertex
     * with the smallest distance (i.e., the head of the list), guaranteeing
     * correct Dijkstra ordering even when bw > minWeight.
     *
     * On road networks, bucket occupancy k ≈ 1-3, so this scan is essentially
     * free. On low-C grids with bw=4 and minW=1, k can be up to ~4 distinct
     * distances per bucket — still very cheap.
     */
    int prev = -1;
    int cur = l0Head[bi];
    ...
    if (prev < 0)
        ...
        pool[prev].next = idx;
#else
    /* LIFO INSERT (same as v3): prepend to head */
 ...
 *
 * Only redistribute entries whose distance falls within the expected
...
/* ----------------------------------------------------------------
 * insertThreeWay — Route a vertex to L0, L1, or cold PQ
 * ---------------------------------------------------------------- */
 ...
 *   - v5 (default):  bw = minW when minW < BW_MULT (adaptive width)
 *   - v5s:           bw = minW * BW_MULT always, but sorted insert
 * ---------------------------------------------------------------- */
...
long OmbiQueue::sssp(const CsrGraph &g, int source)
{
    ...
    /* ================================================================
     * CORRECTNESS FIX: Adaptive bucket width computation
     *
     * v5 (default, no OMBI_V5S):
     *   When minWeight < BW_MULT, use bw = minWeight (effectively BW_MULT=1).
     *   This satisfies the Dinitz bound (bw ≤ minWeight) which guarantees
     *   that no relaxation can produce a same-bucket insertion, so extraction
     *   order within a bucket is irrelevant.
     *   On road networks where minWeight ≥ BW_MULT (typical: minWeight=2+),
     *   this has ZERO performance impact — bw is unchanged.
     *
     * v5s (with OMBI_V5S):
     *   Always use bw = minWeight * BW_MULT (same as v3). Correctness is
     *   guaranteed by sorted insertion instead.
     * ================================================================ */
    long long minW = g.minWeight;
    if (minW < 1) minW = 1;

#ifdef OMBI_V5S
    /* v5s: Keep wide buckets, rely on sorted insert for correctness */
    const long long bw = minW * BW_MULT;
#else
    /* v5: Adaptive — clamp bw to satisfy Dinitz bound when minW is small */
    const long long bw = (minW < (long long)BW_MULT) ? minW : (minW * BW_MULT);
#endif
    ...
    while (l0Total > 0 || l1_any_nonempty(l1Bmp) || !coldPQ.empty())
    {
        ...
        /* --- Step 3: Compare with cold PQ (ALWAYS, like v1/v3) --- */
        while (!coldPQ.empty())
        {
            ...
            if (u < 0 || coldDist < du)
            {
                ...
                /* Push old vertex back directly to L0 */
                if (oldU >= 0)
                {
